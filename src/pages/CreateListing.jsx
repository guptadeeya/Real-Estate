...
//   console.log(formData)
  ...
  return (
    // we change the div section to main to make it seo friendly
    <main className="p-3 max-w-4xl mx-auto">
      <h1 className='text-3xl font-semibold text-center my-7'>Create a listing</h1>

      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
            <input type="text" placeholder='name' className='border p-3 rounded-lg' id='name' maxLength='60' minLength='5' required
            ...
            <div className="flex flex-wrap gap-6">
                ...
                <div className="flex items-center gap-2">
                    <input type="number" id="discountPrice" min='50' max='10000' required className="p-3 border border-gray-300 rounded-lg" onChange={handleChange} value={formData.discountPrice}/>
                    <div className="flex flex-col items-center text-xs">
                    <p>Discount Price</p>
                    <span>($/months)</span>
                    </div>
...

{
    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
        <div key={url} className="flex justify-between p-3 border items-center">
            ...
            {/*we need to have arrow func. while calling otherwise it will be called automatically without clicking on button  */}
        </div>
    ))
}

        <button type="button" className="p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 diabled:opacity-50" onClick={handleSubmit}>
